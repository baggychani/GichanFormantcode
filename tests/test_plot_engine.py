...
class TestPlotEngine(unittest.TestCase):
    ...
    def test_apply_scale(self):
        ...
        np.testing.assert_array_almost_equal(self.engine._apply_scale(data, "bark"), expected_bark)
        
        # Log
        expected_log = hz_to_log(data)
        np.testing.assert_array_almost_equal(self.engine._apply_scale(data, "log"), expected_log)
    ...
    def test_set_ticks_hz_on_bark_scale(self):
        ...
        self.engine._set_ticks(
            ax, "x", scale_type="bark", 
            min_val=500, max_val=2500, 
            step_major=500, step_minor=100, 
        ...
        expected_ticks = [self.engine._apply_scale(h, "bark") for h in [500, 1000, 1500, 2000, 2500]]
    ...
    def test_set_ticks_bark_on_bark_scale(self):
        ...
        self.engine._set_ticks(
            ax, "x", scale_type="bark", 
            min_val=bark_min, max_val=bark_max, 
            step_major=500, step_minor=100, 
            use_bark_units=True
        )
    ...
    def test_draw_plot_axis_units(self):
        ...
        # Hz 단위 표시 설정
        params = {
            "type": "f1_f2", "origin": "top_left", "f1_scale": "linear", "f2_scale": "linear",
            "use_bark_units": False
        }
