...
import { PlotControlRail } from "./interactivePlot/PlotControlRail";
import { PlotInspector } from "./interactivePlot/PlotInspector";
import { PlotStage } from "./interactivePlot/PlotStage";
import { PlotWindowOverlays } from "./interactivePlot/PlotWindowOverlays";
import { PlotWindowHeader } from "./interactivePlot/PlotWindowHeader";
import { useDrawSession } from "./interactivePlot/useDrawSession";
import { useBatchExportSession } from "./interactivePlot/useBatchExportSession";
import { useLayerSession } from "./interactivePlot/useLayerSession";
import { usePlotNavigation } from "./interactivePlot/usePlotNavigation";
import { usePlotWindowActions } from "./interactivePlot/usePlotWindowActions";
import {
  type InteractiveRenderOverrides,
  usePlotRender,
} from "./interactivePlot/usePlotRender";
import { usePlotWindowSession } from "./interactivePlot/usePlotWindowSession";
...
export function InteractivePlotWindow() {
  ...
  const { navigateTo, navigateByPosition } = usePlotNavigation({
    aliveRef,
    navigatingRef,
    currentIndexRef,
    sources,
    currentFileKey,
    analysisUseBark: analysis?.use_bark_units ?? false,
    normalization,
    ranges,
    defaultRanges,
    sigma,
  ...
  }, [combinedVisible, currentIndex, navigateTo, rawCurrentIndex, sources.length]);
  ...
  const {
    ...
    layerOrder,
    lockedLayers,
    drawObjects: currentDrawObjects,
    setMessage,
  });
  ...
  }, [canNavigate, clearLayerSelection, copySelectedLayerDesign, currentSourcePosition, drawArrowHead, drawArrowMode, drawColor, drawEditorOpen, drawLineStyle, drawTool, drawWidth, drawingPoints, exitDrawMode, finishDrawLine, finishDrawPolygon, navigateByPosition, navigating, openComparePlot, pasteLayerDesign, resetTransientRuler, rulerSettingsOpen, saveProject, selectedLayer, selectedLayers, setRulerSettingsOpen, shortcutHelpOpen, sources, toggleDrawMode, toggleLabelMode, toggleRulerMode, tool]);

  const batchExport = useBatchExportSession({
    aliveRef,
  ...
  const analysisProps = {
    ...
    showEllipse,
    onShowEllipseChange: (next: boolean) => {
      setShowEllipse(next);
      void renderInteractive({
        showEllipse: next,
    ...
    onExport: (format: "png" | "jpg" | "svg") => void exportInteractive(format),
    onExportCombinedTxt: () => void exportCombinedTxt(),
    onSaveProject: () => void saveProject(),
    onOpenBatchExport: batchExport.openDialog,
  };
  const globalDesignProps = {
    design,
  ...
  const drawEditorProps = drawEditorOpen ? {
    kind: drawEditorKind,
    mode: drawEditorMode,
    lineDraft,
  ...
  const vowelAnalysisProps = vowelAnalysisOpen ? {
    currentSource,
    sources,
    currentIndex,
    ...
    normalization,
    plotType,
    onNavigate: (index: number) => void navigateTo(index),
  ...
  return (
    <main className={`interactive-plot-workspace ${leftOpen ? "" : "left-is-collapsed"} ${rightOpen ? "" : "right-is-collapsed"}`}>
      <PlotWindowHeader
        ...
        legacyDisabled={busy || !sources.length}
        onOpenShortcutHelp={() => setShortcutHelpOpen(true)}
        onOpenLegacyPlot={() => void openLegacyPlot()}
      />
      ...
      <PlotControlRail
        ...
        onNavigateByPosition={navigateByPosition}
        analysisProps={analysisProps}
        globalDesignProps={globalDesignProps}
      />
      <PlotStage
      ...
      <PlotInspector
        ...
        currentVowelCount={currentVowels.length}
      ...
      />
      <PlotWindowOverlays
        batchExportProps={batchExport.isOpen ? batchExport.dialogProps : null}
        textInput={textInput}
        onTextInputChange={(draft) => {
          if (textInput) setTextInput({ ...textInput, draft });
        }}
        onTextInputClose={() => setTextInput(null)}
        onTextInputConfirm={confirmTextInput}
        drawEditorProps={drawEditorProps}
        vowelAnalysisProps={vowelAnalysisProps}
