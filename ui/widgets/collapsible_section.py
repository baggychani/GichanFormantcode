...
접힘 상태는 위젯 메모리에만 유지합니다(디스크 저장 없음).
→ 새 플롯 창: CollapsibleSection은 펼침, AdvancedOptionsBlock(고급 옵션)은 접힘.
→ 같은 창에서 파일 전환(<- ->): 위젯이 살아 있으므로 접힘 상태 유지.
"""
...
class CollapsibleSection(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self._body_layout.setContentsMargins(0, lc.SPACING_SECTION_FIRST_ITEM_PX, 0, 0)
...
class AdvancedOptionsBlock(QWidget):
    ...
    def _apply_collapsed_state(self) -> None:
        ...
        self._toggle_btn.setText("고급 옵션  ▼" if expanded else "고급 옵션  ▶")
