...
_GITHUB_MARK_SVG_PATH = (
    ...
    "c-1.087-.744.084-.729.084-.729 1.205.082 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 "
    "3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 "
    "1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.266 1.983-.399 "
    "3.003-.404 1.02.005 2.047.138 3.006.404 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 "
    "2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 "
    "2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.686.825.57C20.565 22.092 24 17.592 24 12.297"
    "c0-6.627-5.373-12-12-12"
)


def create_github_mark_icon(
    size: int = 16, color: str = "#ffffff", *, gap_after: int = 0
) -> QIcon:
    """GitHub 마크 아이콘 (SVG 경로를 QtSvg로 렌더).

    gap_after: 버튼 텍스트와의 간격용 오른쪽 투명 여백(px).
    """
    ...
    svg = (
        ...
        f'<path fill="{color}" d="{_GITHUB_MARK_SVG_PATH}"/>'
        f"</svg>"
    )
    renderer = QSvgRenderer(QByteArray(svg.encode("utf-8")))
    app = QApplication.instance()
    dpr = app.primaryScreen().devicePixelRatio() if app else 1.0
    total_w = size + max(0, gap_after)
    pixmap = QPixmap(int(total_w * dpr), int(size * dpr))
    pixmap.setDevicePixelRatio(dpr)
    pixmap.fill(Qt.GlobalColor.transparent)
    painter = QPainter(pixmap)
    try:
